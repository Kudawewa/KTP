...
import { renderFinePrint, splitFinePrint } from "@/utils/finePrint";
...
export default function BookingForm() {
  ...
  const serviceDuration = getServiceDuration(form.service_type);
  ...
  return (
    <>
      <style>{`
        ...
        .bk-btn:hover:not(:disabled) { background: #8f1818; transform: translateY(-2px); }
      ...
      <section id="booking" className="bk-section">
        <div className="bk-inner">
          ...
          <div>
            ...
            <h2 className="bk-headline">Secure Your Appointment</h2>
            <p className="bk-subline">
              Pick a time that works for you. We&apos;ll confirm your slot within the hour
              and have everything ready when you arrive.
            </p>
            <div className="bk-trust">
              {[
                "No deposit required to book",
                "90 minute average turnaround (Subject to availability)",
                "Upfront price no surprises",
                "Free wheel alignment check with every tyre change",
              ].map((item) => {
          ...
          <div className="bk-card">
            {status === "success" ? (
              <div className="bk-success">
                ...
                <h3 style={{ fontSize: "1.4rem", fontWeight: 800, marginBottom: 10 }}>Booking Received!</h3>
                <p style={{ color: "rgba(255,255,255,0.65)", fontSize: "0.97rem" }}>
                  We&apos;ll confirm your appointment shortly. Check your email for details.
                </p>
                {refNumber && (
                  <span className="bk-success__ref">{refNumber}</span>
                )}
                <button
                  onClick={() => setStatus("idle")}
                  style={{ display: "block", margin: "24px auto 0", background: "none", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 6, padding: "8px 20px", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: "0.88rem" }}
                >
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="bk-row">
                  ...
                  <div className="bk-field">
                    <label>Phone *</label>
                    <input name="phone" type="tel" value={form.phone} onChange={handleChange} placeholder="04XX XXX XXX" required />
                  </div>
                </div>
                <div className="bk-row-3">
                  <div className="bk-field">
                    <label>Vehicle Make</label>
                    <input name="vehicle_make" value={form.vehicle_make} onChange={handleChange} placeholder="Toyota" />
                  </div>
                  <div className="bk-field">
                    <label>Model</label>
                    <input name="vehicle_model" value={form.vehicle_model} onChange={handleChange} placeholder="HiLux" />
                  </div>
                  <div className="bk-field">
                    <label>Year</label>
                    <input name="vehicle_year" type="number" min="1990" max="2030" value={form.vehicle_year} onChange={handleChange} placeholder="2021" />
                  </div>
                </div>
                <div className="bk-field">
                ...
                <div className="bk-field">
                  ...
                  {serviceDuration && (
                    <div className="bk-hint">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#c9a84c" strokeWidth="2.5" className="shrink-0">
                        <circle cx="12" cy="12" r="10" />
                        <polyline points="12 6 12 12 16 14" />
                      </svg>
                      <span>
                        Allow about {serviceDuration}
                        {renderFinePrint(" (Subject to availability)")}
                      </span>
                    </div>
                  )}
                </div>
                <div className="bk-row">
                  <div className="bk-field">
                    ...
                    <input name="preferred_date" type="date" value={form.preferred_date} onChange={handleChange} min={new Date().toISOString().split("T")[0]} />
                  </div>
                  <div className="bk-field">
                ...
                <div className="bk-field">
                  ...
                  <textarea name="notes" value={form.notes} onChange={handleChange} placeholder="Anything else we should know — tyre size, specific issue, etc." />
                </div>
                <button type="submit" className="bk-btn" disabled={status === "loading"}>
                  {status === "loading" ? (
                    "Submitting…"
                  ) : (
                    <>
                ...
                {status === "error" && (
                  <p className="bk-error">
                    Something went wrong. Please try again or call us on (08) 9359 1444.
                  </p>
                )}
              </form>
            )}
